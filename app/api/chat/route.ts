export async function POST(request:Request){
 ...
 const upstream=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${bridgeToken}`},body:JSON.stringify({messages:data.messages,discoveredProjects:data.discoveredProjects}),signal:AbortSignal.timeout(105000),redirect:'manual'});
 if(!upstream.ok)return respond(upstream.status===429?`${Array.isArray(data.discoveredProjects)?'小津':'小齐'}正在回答其他问题，请稍后重试。`:'本机 Codex 暂时不可用，请稍后重试。',upstream.status===429?429:502);
