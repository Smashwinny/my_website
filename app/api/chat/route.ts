export async function POST(request:Request){
 ...
 if(!url||!bridgeToken||!visitorToken)return respond('小星还没连接本机 Codex，请站主先配置连接服务。',503);
 ...
 if(!upstream.ok)return respond(upstream.status===429?'小星正在回答其他问题，请稍后重试。':'本机 Codex 暂时不可用，请稍后重试。',upstream.status===429?429:502);
