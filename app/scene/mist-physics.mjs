...
export class MistController{
 constructor(index=0){this.position=islandPosition(index,0);this.velocity=new Vector3();this.time=0;this.accumulator=0;this.grounded=true;this.island=index;this.checkpoint=index;this.coyote=.1;this.buffer=0;this.jumpsUsed=0;this.landing=0;this.respawns=0}
 requestJump(){if(this.jumpsUsed<2)this.buffer=.14}
 reset(){this.position.copy(islandPosition(this.checkpoint,this.time));this.velocity.set(0,0,0);this.island=this.checkpoint;this.grounded=true;this.buffer=0;this.jumpsUsed=0;this.coyote=.1;this.respawns++}
 ...
 step(dt,direction){
  ...
  if(this.buffer>0&&this.jumpsUsed<2){
   // Falling off an island still leaves one air jump to recover.
   this.jumpsUsed=this.coyote>0?1:2;this.velocity.y=9;this.buffer=0;this.coyote=0;this.grounded=false;
  }
