...
export type Motion={speed:number;metersPerSecond?:number;grounded:boolean;verticalVelocity:number;landing:number;turn:number;engaged:boolean};
...
export async function loadCharacter(companion=false,source='/models/traveler.vrm',mist=false,onProgress:(progress:CharacterProgress)=>void=()=>{}):Promise<Character>{
 ...
 // Follow global travel in character space. Head/body motion still excites hair,
 // without a constant walking velocity stretching long hair horizontally.
 if(!source.startsWith('blob:')&&vrm.springBoneManager){for(const joint of vrm.springBoneManager.joints)if(!joint.center)joint.center=root;vrm.springBoneManager.setInitState()}
 if(mist){vrm.scene.traverse(o=>{if(o instanceof THREE.Mesh){const convert=(material:THREE.Material)=>{const old=material as THREE.MeshStandardMaterial;const m=new THREE.MeshStandardMaterial({name:old.name,map:old.map,color:old.color?.clone()??new THREE.Color('white'),normalMap:old.normalMap,roughness:.92,side:old.side,alphaTest:old.alphaTest,transparent:old.transparent,opacity:old.opacity});if(/HAIR/i.test(m.name)){m.color.set('#817567')}if(/Tops|Bottoms|Shoes|cloth/i.test(m.name)){m.color.set('#aaa08c')}old.dispose();return m};o.material=Array.isArray(o.material)?o.material.map(convert):convert(o.material)}})}
 ...
 const headDecor=new THREE.Group();head.add(headDecor);chest.add(decor);if(vrm.meta.metaVersion==='0'){headDecor.rotation.y=Math.PI;decor.rotation.y=Math.PI}decor.add(wingLeft,wingRight);
 ...
 const locomotion=companion?null:createLocomotion(vrm,root);
 ...
 phase=locomotion?locomotion.cycle*Math.PI*2:phase+delta*(2+speed*7)*speed;const step=-Math.cos(phase),opposite=-step,breath=Math.sin(time*2.1),hover=Math.sin(time*2.4);
 if(companion){
  ...
  pose('hips',0,0,0);pose('spine',-.035*speed+.1*land);pose('chest',breath*.012+speed*.04,-step*speed*.055,-turn*.08);
  ...
  locomotion?.update(delta,motion);
 }
