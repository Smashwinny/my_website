...
export class PlayerController {
 ...
 reset(position){this.position.copy(position);this.capsule.start.copy(position).y+=this.radius;this.capsule.end.copy(position).y+=this.height-this.radius;this.velocity.set(0,0,0);this.jumpBuffer=0;this.jumpsUsed=0;this.coyote=0;this.grounded=false;this.accumulator=0}
 requestJump(){if(this.jumpsUsed<2)this.jumpBuffer=.12}
 ...
 step(dt,direction){
  ...
  if(this.jumpBuffer>0&&this.jumpsUsed<2){
   // After walking off a ledge, only the air jump remains outside coyote time.
   this.jumpsUsed=this.coyote>0?1:2;this.velocity.y=8;this.grounded=false;this.coyote=0;this.jumpBuffer=0;
  }
