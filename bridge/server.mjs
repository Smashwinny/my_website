...
export function createBridge({token=process.env.COMPANION_BRIDGE_TOKEN,run=runCodex,origins=(process.env.COMPANION_ALLOWED_ORIGINS||'').split(',').filter(Boolean)}={}){
 ...
 if(!validateMessages(data.messages))return reply(400,{error:'Invalid messages'});
 ...
 recent.push(Date.now());active=true;try{reply(200,{reply:await run(data.messages)})}catch{reply(502,{error:'Codex unavailable; check dedicated login and bridge logs'})}finally{active=false}
 });
}
