...
在「切换风格」选择「回声之庭 · 纪念碑谷风格」。固定正交镜头、月白高塔、绯色拱廊与碧水庭组成原创建筑；小旅人沿真实台阶行走。参考 [ustwo 官方介绍](https://ustwogames.co.uk/our-games/monument-valley/) 的极简建筑与可操纵路径方向，不使用游戏角色、关卡、图片、音乐或商标作为素材，也不声称复刻其不可能几何玩法。

点击庭院圆形落点移动；先到「回转之心」，按 R 或点击「转桥 90°」改变东西／南北连接。C 键或「走到回转之心」按钮返回中央；每座庭院的相邻展台通过机关区的具名按钮抵达。手机使用同样的点击操作。途中禁止转桥，打开弹窗和切换到后台会暂停旅人和机关；「回到起点」恢复初始路径。

八个展台来自 `app/scene/monument-paths.mjs`，按项目索引余数分配作品；超过八个项目时，同一展台可查看多个作品，不会丢失条目。新的 Three.js 场景按需加载，不下载人物 VRM、额外贴图或外部游戏资产。WebGL 不可用时提供直接作品列表。

测试覆盖八个展台往返、断桥连接与转动锁定、阶梯平台衔接、手机目标间距和资源释放；不替代实机观感与帧率验收。本机向导桥接增加 `monument` 白名单与操作说明；已运行的独立桥接拉取新版并重启后才使用新主题上下文。站点兼容尚未更新的桥接：只有收到明确的主题校验拒绝时，才重试通用作品对话，不改变鉴权或跟随重定向。
